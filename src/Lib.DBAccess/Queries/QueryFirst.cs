...
{
    #region Oracle
    public static Task<DatabaseQueryFirstResponse<T>> QueryFirstAsync<T>(
    ...
    private static async Task<DatabaseQueryFirstResponse<T>> ExecuteQueryAsync<T>(
        ...
        where T : class, new()
    {
        try
        {
            ...
            await using (reader.ConfigureAwait(false))
            {
                ...
                return new DatabaseQueryFirstResponse<T>
                {
                    ...
                    ErrorCode = errorCode,
                    Data = hasData ? data : throw new NoDatabaseRowReturnedException("No row found in Database response"),
                };
            }
        }
        catch (Exception ex)
        {
            return DatabaseQueryFirstResponse.Failure<T>(ex.Message);
        }
    }
        ...
        where T : class, new()
    {
        try
        {
            ...
            return new DatabaseQueryFirstResponse<T>
            {
                ...
                ErrorMessage = errorMessage ?? DefaultConstants.NoErrorMessage,
                ErrorCode = errorCode,
                Data = hasData ? data : throw new NoDatabaseRowReturnedException("No row found"),
            };
        }
        catch (Exception ex)
        {
    ...
    #region MySql
    public static async Task<DatabaseQueryFirstResponse<T>> QueryFirstAsync<T>(
